...

{
    ...
    function chat(){
        ...
        if (empty(trim($this->session->userdata['status']))) {
            ...
            $data['main_content'] = 'admin/chat_view';
...
srp_employeesdetails.Ename2 FROM property pro LEFT JOIN bed_types bt ON bt.bed_type_id = pro.bed_type_id Left JOIN srp_employeesdetails ON pro.agent_id = srp_employeesdetails.EIdNo WHERE pro.property_id = {$property_id}")->row_array();
    ...
    function getCity(){
        $CountryID= trim($this->input->post('CountryName'));

        $result = $this->property_model->load_property_by_country_wise_city($CountryID);
    ...
    function save_chat_message(){ 
        ...
        $data['property_id']=$_REQUEST['HiddenPrpertyId'];
        $data['argent_id']=$_REQUEST['ArgentID'];
        $data['user_id']=$_REQUEST['userID'];
        $data['message']=$_REQUEST['Message'];
      
        $this->property_model->insert_chat_message($data);
    ...
    function Lode_chat_history(){
        ...
        $HiddenPrpertyId=$_REQUEST['HiddenPrpertyId'];
        $ArgentID=   $_REQUEST['ArgentID'];
        $userID=  $_REQUEST['userID'];

        $result = $this->property_model->load_chat_history($HiddenPrpertyId,$ArgentID, $userID);
