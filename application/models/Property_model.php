...

{
    ...
    function insert_chat_message($data){
        ...
        $result = $this->db->insert('chat_history', $data);
    ...
    function load_chat_history($HiddenPrpertyId,$ArgentID, $userID){
        $result = "";

        $CI =& get_instance();

        $CI->db->select('se.*,srp_employeesdetails.Ename1');
        ...
        $CI->db->where('se.property_id', $HiddenPrpertyId);

        $CI->db->where('se.user_id', $userID);

        $CI->db->where('se.argent_id', $ArgentID);

        $CI->db->order_by('se.createdDate', 'asc');
